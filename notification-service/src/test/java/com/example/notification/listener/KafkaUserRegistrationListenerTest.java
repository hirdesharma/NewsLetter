...
class KafkaUserRegistrationListenerTest {
  ...
  void testListenerSuccess() throws Exception {
    String data = "{\"userId\":\"12345\",\"email\":\"test@example.com\"}";

    JsonNode jsonNode = mock(JsonNode.class);
    when(objectMapper.readTree(data)).thenReturn(jsonNode);
    when(jsonNode.get("userId")).thenReturn(mock(JsonNode.class));
    when(jsonNode.get("userId").asText()).thenReturn("12345");
